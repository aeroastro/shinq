...
module Shinq
  class Client
    ...
    def self.schedulable?(table_name:)
      @schedulability_by_table_name ||= {}
      @schedulability_by_table_name[table_name] ||= check_schedulability(table_name: table_name)
    end

    def self.check_schedulability(table_name:)
      quoted = SQL::Maker::Quoting.quote(table_name)
      column = Shinq.connection.query(<<-EOS).to_a.first
select column_name from information_schema.columns where table_schema = database() and table_name = #{quoted} and column_name = 'scheduled_at'
      EOS
      column.present?
    end
